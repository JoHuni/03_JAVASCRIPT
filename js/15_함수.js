...
btn1.addEventListener("click", function(){
    ...
    //input1에 값이 입력이 안 되었거나 빈칸만 입력된 경우
...
document.querySelector('#btn2a').addEventListener("click", function(){
    ...
    /* 참조란? 
        ...
        변수는 값을 하나만 저장할 수 있기 때문에
        object를 메모리 다른 곳에 저장하고 저장된 곳의 주소를
        변수에 저장해서 필요할 때마다 해당 주소를 찾아가
    ...
    */
    ...
    // -> 함수 수행 후 돌아와서도 참조하고 있던
    // 배열의 2번 인덱스 값이 변화되어 있음
    console.log("arr2[2] : ", arr2[2]);
});

function btn2bFn(el){
    // 매개 변수el(element) : 이벤트가 발생한 요소(HTML에서 this)
...
function pow(num, x){
    let result = 1; //곱했을 때 결과에 영향을 주지 않기 위해 값 1로 초기화
    for(let i = 0; i < x; i++){
        result *= num;
    }
    return result;
...
arrowList[3].addEventListener("click", () => {
    printObject((name, age) => {
        return{"name" : name, "age" : age} //js 객체 모양(딕셔너리)
    });
});

function printObject(otherFn){
    const obj = otherFn("조훈희", 24);
